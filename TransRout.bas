...
'Create A region to shape the Form
    Width = FrmTest.ScaleX(FrmTest.Width, vbTwips, vbPixels)
    Height = FrmTest.ScaleY(FrmTest.Height, vbTwips, vbPixels)
'Create a new Region
    rgnMain = CreateRectRgn(0, 0, Width, Height)
    dcMain = CreateCompatibleDC(FrmTest.hDC)
'Get the picture we us for this calculation
    bmpMain = SelectObject(dcMain, FrmTest.Picture.Handle)
...
'Get the Region Data so we can store it later
    If rgnMain <> 0 Then
        ...
        End If
'Shape the form
        SetWindowRgn FrmTest.hWnd, rgnMain, True
    End If
